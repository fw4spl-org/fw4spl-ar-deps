...
//                        m_patt_id, 1));            
    ...
    for (int j = 0; j < config->marker_num; j++)
        // PATCH : Original code config_patt_id.push_back(std::make_pair<int, int>(j, config->marker[j].patt_id));
            ...
            if (c_iter != config_patt_id.end())
                do {
                    ...
                    if (marker_info[m].id == patt_id) {
                        // PATCH : Original code m2c_idx.insert(std::make_pair<int, int>(m, (*c_iter).first));
